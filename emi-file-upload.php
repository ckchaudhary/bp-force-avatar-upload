...
function emi_fu_component_shortcode($atts) {
	...
	$html .= "<button class='" . $class_attr . "' id='emi_fu_trigger-" . rand() . "' $data_attributes >" . __( 'Upload Image', 'bp-force-avatar-upload' ) . "</button>";
	$html .= "</div>";
	return $html;
}

add_action('wp_ajax_bp_avatar_upload', 'emi_fu_ajax_upload_file', 2);

function emi_fu_ajax_upload_file() {
	//since we are hijacking someone else's action,
	//we need to make sure we are intercepting only our request,
	//if otherwise, let is pass through
	if( !isset( $_POST['emi_fu_key'] ) || $_POST['emi_fu_key']!='emi_fu' )
		return;
	...
	//validation
	$retval = emi_fu_parse_file_errors($_FILES['file'], $permissible_mime_types);
	...
	$retval['status'] = bp_core_avatar_handle_upload( $_FILES, 'emi_fu_loggedin_user_avatar_upload_dir' );
	if( $retval['status'] ){
		global $bp;
		$retval['message'] = array( 'url' => $bp->avatar_admin->image->url );
		...
		$retval['message']['full_width'] = $full_width;
		$retval['message']['full_height'] = $full_height;
		$retval['message']['image0'] = $image[0];
		$retval['message']['image1'] = $image[1];
		$retval['message']['crop'] = array(
...
function emi_fu_loggedin_user_avatar_upload_dir(){
	return xprofile_avatar_upload_dir(false, bp_loggedin_user_id());
}
...
function emi_fu_parse_file_errors($file = '', $mime_types = '') {
	...
	if ($file['error']) {
		$result['status'] = false;
		$result['message'] = __( "No file uploaded or there was an upload error!", "bp-force-avatar-upload" );
		return $result;
	}
	...
	if (!in_array($image_data['mime'], $mime_types)) {
		...
		$result['message'] = sprintf( __( 'The uploaded file must be either of the following : %s!', 'bp-force-avatar-upload' ), $mime_types_csv );
	} elseif (($file['size'] > $max_upload_size)) {
		$result['message'] = sprintt( __( 'The uploaded file was %s bytes! It must not exceed %s bytes.', 'bp-force-avatar-upload' ), $file['size'], $max_upload_size );
	}
